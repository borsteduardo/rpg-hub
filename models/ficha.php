<?php
    ...
    class Ficha {
        ...
        /**
         * Busca a ficha de um usuário específico em uma campanha específica.
         * Usado pelo ver_ficha_controller para carregar o dossiê do investigador.
         *
         * @param int $id_usuario
         * @param int $id_campanha
         * @return array|false
         */
        public function buscarPorUsuarioECampanha(int $id_usuario, int $id_campanha): array|false {
            $stmt = $this->pdo->prepare(
                "SELECT f.*, c.nome AS nome_campanha, c.sistema 
                FROM fichas f
                JOIN campanhas c ON f.id_campanha = c.id
                WHERE f.id_usuario = :id_usuario AND f.id_campanha = :id_campanha
                LIMIT 1"
            );
            $stmt->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
            $stmt->bindParam(':id_campanha', $id_campanha, PDO::PARAM_INT);
